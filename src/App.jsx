...
const BASE_STORAGE_KEY = "srna-command-center-summer-2026-v3";
function getStorageKey(userId) {
  return userId ? `${BASE_STORAGE_KEY}-${userId}` : `${BASE_STORAGE_KEY}-guest`;
}
function loadSavedState(userId) {
  try {
    const raw = localStorage.getItem(ge JSON.stringify(state));
    return true;
  } catch {
    return false;
  }
}
function clearSavedState(userId) {
  try {
...
export default function SRNACommandCenter() {
  ...
  const [assignments, setAssignments] = useState(() => mergeAssignilter, setFilter] = useState(savedState?.filter || "All");
    ...
    async function loadCloudPlanner() {
      try {
        ...
        if (snapshot.exists()) {
          ...
          setActiveTab(cloud.activeTab || "calendar")receptor: "", cases: "", airway: "", meds: "", learning: "", review: "" });
          ...
          const userLocal = loadSavedState(user.uid);
          if (userLocal) {
            setAssignments(mergeAssignmentsWithDefaults(userLocal.assignments || []));
            setFilter(userLocal.filter || "All");
            setActiveTab(userLocal.activeTab || "calendar");
            setClinic"", meds: "", learning: "", review: "" });
            setCustomClinicalSchedule(userLocal.customClinicalSchedule || defaultClinicalSchedule);
            setChecked(userLocal.checked || {});
            setCalendarChecked(userLocal.calendarChecked || {});
            setWeeklyChecked(userLocal.weeklyChecked || {});
            setBrainDump(userLocal.brainDump || ["", "", "", "", ""]);
            setCloudStatus("Cloud sync connected. Loaded your saved local planner and will save it to your account.");
          } else {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      try { await setDoc(getPlannerDocRef(user.uid), { ...plannerState, ownerUid: user.uid, ownerEmail: user.email || "", updatedAt: serverTimestamp() }, { merge: true }); setCloudStatus("Cloud sync saved to tosave still works."); }
    }, 900);
  ...
  const exportPlanner = () =iveTab, clinical, checked, calendarChecked, weeklyChecked, brainDump, customClinicalSchedule, exportedAt: new Date().toISOString() }); setCopyMessage("Planner backup downloaded."); };
  ...
  return <div className="min-h-screen bg-slate-50 p-4 text-slate-900 md:p-8"><div className="mx-auto max-w-7xl space-y-6">
    <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="mb-2 text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Summer 2026</p><h1 className="text-3xl font-bold tracking-tight md:text-5xl">SRNA Command Center</h1><p className="mt-2 max-w-2xl text-slate-600">Interactive calendar, assignment list, clinical template, and weekly brain dump. Each signed-in student gets a private editable schedule tied to their own Google account.</p></div><div className="grid grid-cols-4 gap-3 md:w-[600px]"><Card className="p-4"><div className="text-xs text-slate-500">Assignments</div><div className="mt-1 text-2xl font-bold">{completion}%</div></Card><Card className="p-4"><div className="text-xs text-slate-500">Active</div><div className="mt-1 text-2xl font-bold">{activeCdiv className="text-xs text-slate-500">Calendar Done</div><div className="mt-1 text-2xl font-bold">{calendarItemCount}</div></Card></div></header>
